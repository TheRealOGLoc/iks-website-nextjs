  ...
  return (
    <div className="p-5">
      ...
      <div className="flex flex-wrap flex-col md:flex-row justify-between max-w-[1000px] m-[auto] mt-5">
        { componentData.card && componentData.card.map((content : any, index: number) => <IndustryCard title={content.title} description={content.description} image={content.image} showButton={content.showButton} buttonText={content.buttonText} buttonUrl={content.buttonUrl} key={index} />) }
      </div>
    </div>
  )
}
